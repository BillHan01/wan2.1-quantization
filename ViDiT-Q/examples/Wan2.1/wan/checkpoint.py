...
def load_sharded_model_for_inference(model, model_dir):
    """Load FSDP sharded model state_dict for inference (no optimizer)."""
    with FSDP.state_dict_type(model, StateDictType.SHARDED_STATE_DICT, ShardedStateDictConfig()):
        state_dict_container = {"model": model.state_dict()}
        ...
        model.load_state_dict(state_dict_container["model"])

    print(f"[Rank {torch.distributed.get_rank()}] ✅ Loaded model from {model_dir}")
    return model
