...
def main():
    ...
    if prompts is None:
        if cfg.get("prompt_path", None) is not None:
            prompts = load_prompts("/cv/hanjiarui/code/ViDiT-Q/examples/opensora1.2/assets/t2v_samples.txt", start_idx, cfg.get("end_index", None))
        else:
    ...
    for i in progress_wrap(range(0, len(prompts), batch_size)):
        ...
        for k in range(num_sample):
            ...
            torch.cuda.synchronize()  # 保证 CUDA 操作也计入
            end_time = time.time()
            print(f"[Timing] Loop sampling time: {end_time - start_time:.2f} seconds")
            # == save samples ==
            # if is_main_process():
