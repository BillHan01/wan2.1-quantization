...
def distill_one_step(
        ...
        for s in range(6):
            with torch.no_grad():
                with torch.autocast("cuda", dtype=torch.bfloat16):
                    ...
                    # else:
                    # timesteps_prev = timesteps_all[int(index.item()) + 1 + s]

                    target_pred = teacher_transformer(x_prev_list, timesteps_prev[s],None,max_seq_len,batch_context=encoder_hidden_states,context_mask=encoder_attention_mask,guidance=guidance_tensor)[0] #43200
    ...
    if debug_save_dir:
        ...
        with torch.no_grad():
            for value,name in [(latents, "latents"), (model_pred_prev, "model_pred_prev"), (target_prev, "target_prev")]:
...
def main(args):
    ...
    for step in range(init_steps + 1, args.max_train_steps + 1):
        ...
        if DEBUG:
            ...
            debug_save_dir = f"/vepfs-zulution/wangxuekuan/code/fast-video/Wan2.1/outputs/s5_exp0_debug/{step}"
